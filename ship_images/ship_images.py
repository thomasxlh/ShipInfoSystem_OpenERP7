...
class ship_images(orm.Model):
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(ship_images, self).unlink(cr, uid, ids, context=context)

    def create(self, cr, uid, vals, context=None):
        if vals.get('name') and not vals.get('extension'):
            vals['name'], vals['extension'] = os.path.splitext(vals['name'])
        return super(ship_images, self).create(cr, uid, vals, context=context)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        return super(ship_images, self).write(cr, uid, upd_ids, vals, context=context)
    ...
    def get_image(self, cr, uid, id, context=None):
        ...
        if image.link:
            ...
            try:
                if isinstance(image.ship_table_id.ship_id, bool):
                    _logger.debug('ship not completely setup, no image available')
                    full_path = False
                else:
                    full_path = self._image_path(cr, uid, image, context=context)
            except Exception, e:
    ...
    _columns = {
        'name': fields.char(u'图片名称', required=True),
        'extension': fields.char(u'文件格式', oldname='extention'),
        ...
        'comments': fields.text(u'图片备注'),
        'ship_table_id': fields.many2one('ship.info', 'Ship')
    }
    ...
    _sql_constraints = [('uniq_name_ship_table_id',
                         'UNIQUE(ship_table_id, name)',
                         _('A ship can have only one '
